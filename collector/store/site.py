...
function card(ev,t){const color=CAT[ev.type]||DEF,kind=(ev.audience==='B2B')?'b2b':(ev.kind==='临时'&&ev.type==='演出')?'show':(KMAP[ev.kind]||'live');
 ...
 const attrs=`style="border-left-color:${color}" data-kind="${kind}" data-type="${esc(ev.type)}" data-kid="${ev.kid_friendly?1:0}" data-new="${isNew?1:0}" data-unfit="${ev.kid_unfit?1:0}" data-days="${days}" data-search="${sdata}"`;
...
function setTab(b,t){curTab=t;showFar=false;document.getElementById('q').value='';mark('.tabs',b);apply();}
...
function render(data){const evts=(data.events||[]).slice();
 ...
 const liveN=evts.filter(e=>e.kind==='临时'&&e.type!=='演出'&&e.audience!=='B2B').length;
 const b2bN=evts.filter(e=>e.audience==='B2B').length;
 document.getElementById('sub').textContent='近期'+liveN+' · 演出'+showN+' · 年度'+evts.filter(e=>e.kind==='年度固定').length+' · 场馆'+evts.filter(e=>e.kind==='固定场馆').length+' · 行业'+b2bN+' · 🆕'+nNew;
